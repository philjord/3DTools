...
/** Copied from Jogamp as the auto look aside doesn't work with the Mavenized version of Jogl the atomic 
 * folder is not auto loaded, this simply removes the sibling look up part.
 * To use this class requires "jogl-fonts-p0.jar" to be in the class path
 */
public class UbuntuFontLoader implements FontSet {
    ...
    private synchronized Font abspath(final String fname, final int family, final int style) throws IOException {
        if( !attemptedJARLoading ) {
            ...
            //**************  removed to avoid extraneous output to log of unloadable jar file
