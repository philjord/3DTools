...
public abstract class Frustum
{
	 ...
	 */
	public Frustum(int numCanvases)
	{
	...
	public BoundingPolytope getBoundingPolytope(int canvasId)
	{
		frustums[canvasId].bounds.setPlanes(frustums[canvasId].frustumPlanes);
		return frustums[canvasId].bounds;
	}
	 ...
	 */
	public int isTriangleInFrustum(float p1X, float p1Y, float p1Z, float p2X, float p2Y, float p2Z, float p3X, float p3Y, float p3Z)
	{
	 ...
	 */
	protected abstract void getInverseWorldProjection(int id, Matrix4d matrix);
	 ...
	 */
	protected void updatePlanes(int canvasId)
	{
		// Now compute the 6 plane equations
		// left
		computePlaneEq(frustumPoints[0], frustumPoints[4], frustumPoints[5], frustumPoints[1], frustums[canvasId].frustumPlanes[0]);

		// right
		computePlaneEq(frustumPoints[3], frustumPoints[2], frustumPoints[6], frustumPoints[7], frustums[canvasId].frustumPlanes[1]);

		// top
		computePlaneEq(frustumPoints[1], frustumPoints[5], frustumPoints[6], frustumPoints[2], frustums[canvasId].frustumPlanes[2]);

		// bottom
		computePlaneEq(frustumPoints[0], frustumPoints[3], frustumPoints[7], frustumPoints[4], frustums[canvasId].frustumPlanes[3]);

		// front
		computePlaneEq(frustumPoints[0], frustumPoints[1], frustumPoints[2], frustumPoints[3], frustums[canvasId].frustumPlanes[4]);

		// back
		computePlaneEq(frustumPoints[4], frustumPoints[7], frustumPoints[6], frustumPoints[5], frustums[canvasId].frustumPlanes[5]);
	}
	 ...
	 */
	protected void computePlaneEq(Point4d p1, Point4d p2, Point4d p3, Point4d p4, Vector4d planeEq)
	{
		tVec1.x = p3.x - p1.x;
		tVec1.y = p3.y - p1.y;
		tVec1.z = p3.z - p1.z;
	...
	{
		return "" + this.getClass().getSimpleName() + " " + frustums[0];
	}
}
