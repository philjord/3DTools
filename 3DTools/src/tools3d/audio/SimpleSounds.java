...
/**
 * NOTE! this string is a URL this is how you specify a relative url. MediaContainer sample = new
 * MediaContainer("file:media/sounds/factCREAK.au");
 *
...
public class SimpleSounds {
	...
	/**
	 * Not to be used for background sounds! use the system supplied direct mp3 play capacity, this guy is for
	 * spatialized sounds only! JOAL error code: 40961 seems fine?
	 */
	...
	public static BranchGroup createPointSoundMp3(	InputStream is, float maximumAttenuationDistance, int loopCount,
													float gain) {
		ConverterBB conv = new ConverterBB();
		int detail = Converter.PrintWriterProgressListener.VERBOSE_DETAIL;
		ConverterBB.ProgressListener listener = new ConverterBB.PrintWriterProgressListener(
	...
	public static BranchGroup createPointSound(	MediaContainer mc, float maximumAttenuationDistance, int loopCount,
		...
		ps.setDistanceGain(new float[] {0, minimumAttenuationDistance, maximumAttenuationDistance},
				new float[] {maxGain, maxGain, 0});
		ps.setEnable(true);
		ps.setPause(false);
	...
	/**
	 * FIXME: dump this rubbish, expensive and pointless use teh system provided mp3 players
	 * @param is
	 * @param loopCount
	 * @param gain
	 * @return
