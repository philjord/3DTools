...
class JoglPipeline extends Pipeline
{
			...
			float[] varray, float[] carray, int cDirty)
	{
		...
		executeGeometryArray(ctx, geo, geo_type, isNonUniformScale, useAlpha, ignoreVertexColors, startVIndex, vcount, vformat,
				texCoordSetCount, texCoordSetMap, texCoordSetMapLen, texUnitOffset, numActiveTexUnitState, vertexAttrCount,
				vertexAttrSizes, varray, null, carray, cDirty);
	}
	...
	void executeVA(Context ctx, GeometryArrayRetained geo, int geo_type, boolean isNonUniformScale, boolean ignoreVertexColors, int vcount,
			...
			float[][] vertexAttrData, int texCoordMapLength, int[] texcoordoffset, int numActiveTexUnitState, int[] texIndex,
			int texstride, Object[] texCoords, int cdirty)
	{
		if (VERBOSE)
	...
	void executeVABuffer(Context ctx, GeometryArrayRetained geo, int geo_type, boolean isNonUniformScale, boolean ignoreVertexColors,
			...
			int[] vertexAttrIndices, Object[] vertexAttrData, int texCoordMapLength, int[] texcoordoffset, int numActiveTexUnitState,
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.disableGlobalAlpha()");
		// FIXME: SUN_global_alpha
			...
			for (int i = 0; i < sarray.length; i++)
			{
				...
				for (int j = 0; j < sarray[i]; j++)
				{
					if ((vformat & GeometryArray.NORMALS) != 0)
					{
						if (nxform != null)
						{
							float nx = (float) (nxform[0] * varray[normoff] + nxform[1] * varray[normoff + 1] + nxform[2]
									* varray[normoff + 2]);
							float ny = (float) (nxform[4] * varray[normoff] + nxform[5] * varray[normoff + 1] + nxform[6]
									* varray[normoff + 2]);
							float nz = (float) (nxform[8] * varray[normoff] + nxform[9] * varray[normoff + 1] + nxform[10]
									* varray[normoff + 2]);
					...
					if ((vformat & GeometryArray.COORDINATES) != 0)
					{
						if (xform != null)
						{
							...
							float w = (float) (xform[12] * varray[coordoff] + xform[13] * varray[coordoff + 1] + xform[14]
									* varray[coordoff + 2] + xform[15]);
							float winv = 1.0f / w;
							float vx = (float) (xform[0] * varray[coordoff] + xform[1] * varray[coordoff + 1] + xform[2]
									* varray[coordoff + 2] + xform[3])
									* winv;
							float vy = (float) (xform[4] * varray[coordoff] + xform[5] * varray[coordoff + 1] + xform[6]
									* varray[coordoff + 2] + xform[7])
									* winv;
							float vz = (float) (xform[8] * varray[coordoff] + xform[9] * varray[coordoff + 1] + xform[10]
									* varray[coordoff + 2] + xform[11])
									* winv;
				...
				|| (geo_type == GeometryRetained.GEO_TYPE_POINT_SET) || (geo_type == GeometryRetained.GEO_TYPE_LINE_SET))
		{
			...
			for (int j = 0; j < vcount; j++)
			{
				if ((vformat & GeometryArray.NORMALS) != 0)
				{
					if (nxform != null)
					{
						float nx = (float) (nxform[0] * varray[normoff] + nxform[1] * varray[normoff + 1] + nxform[2] * varray[normoff + 2]);
						float ny = (float) (nxform[4] * varray[normoff] + nxform[5] * varray[normoff + 1] + nxform[6] * varray[normoff + 2]);
						float nz = (float) (nxform[8] * varray[normoff] + nxform[9] * varray[normoff + 1] + nxform[10]
								* varray[normoff + 2]);
						gl.glNormal3f(nx, ny, nz);
					}
				...
				if ((vformat & GeometryArray.COORDINATES) != 0)
				{
					if (xform != null)
					{
						...
						float w = (float) (xform[12] * varray[coordoff] + xform[13] * varray[coordoff + 1] + xform[14]
								* varray[coordoff + 2] + xform[15]);
						float winv = 1.0f / w;
						float vx = (float) (xform[0] * varray[coordoff] + xform[1] * varray[coordoff + 1] + xform[2] * varray[coordoff + 2] + xform[3])
								* winv;
						float vy = (float) (xform[4] * varray[coordoff] + xform[5] * varray[coordoff + 1] + xform[6] * varray[coordoff + 2] + xform[7])
								* winv;
						float vz = (float) (xform[8] * varray[coordoff] + xform[9] * varray[coordoff + 1] + xform[10]
								* varray[coordoff + 2] + xform[11])
								* winv;
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.buildGAForByRef()");
		...
		if (floatCoordDefined)
		{
			...
			if (xform != null)
			{
				...
				{
					fverts.put(i, (float) (xform[0] * vfcoords[i] + xform[1] * vfcoords[i + 1] + xform[2] * vfcoords[i + 2]));
					fverts.put(i + 1, (float) (xform[4] * vfcoords[i] + xform[5] * vfcoords[i + 1] + xform[6] * vfcoords[i + 2]));
					fverts.put(i + 2, (float) (xform[8] * vfcoords[i] + xform[9] * vfcoords[i + 1] + xform[10] * vfcoords[i + 2]));
				}
			}
		}
		else if (doubleCoordDefined)
		{
			...
			if (xform != null)
			{
				...
				for (int i = initialCoordIndex; i < vcount * 3; i += 3)
				{
					dverts.put(i, (xform[0] * vdcoords[i] + xform[1] * vdcoords[i + 1] + xform[2] * vdcoords[i + 2]));
					dverts.put(i + 1, (xform[4] * vdcoords[i] + xform[5] * vdcoords[i + 1] + xform[6] * vdcoords[i + 2]));
					dverts.put(i + 2, (xform[8] * vdcoords[i] + xform[9] * vdcoords[i + 1] + xform[10] * vdcoords[i + 2]));
				}
			}
		}
		else
		...
		if (normalsDefined)
		{
			...
			if (nxform != null)
			{
				...
				{
					norms.put(i, (float) (nxform[0] * ndata[i] + nxform[1] * ndata[i + 1] + nxform[2] * ndata[i + 2]));
					norms.put(i + 1, (float) (nxform[4] * ndata[i] + nxform[5] * ndata[i + 1] + nxform[6] * ndata[i + 2]));
					norms.put(i + 2, (float) (nxform[8] * ndata[i] + nxform[9] * ndata[i + 1] + nxform[10] * ndata[i + 2]));
				}
			}
		}
		else
		...
		executeGeometryArrayVA(ctx, geo, geo_type, isNonUniformScale, ignoreVertexColors, vcount, vformat, vdefined, initialCoordIndex,
				fverts, dverts, initialColorIndex, fclrs, bclrs, initialNormalIndex, norms, vertexAttrCount, vertexAttrSizes,
				vertexAttrIndices, vertexAttrBufs, texCoordMapLength, tcoordsetmap, texCoordMapLength, texIndices, texStride, texCoordBufs,
	...
	private void testForInterleavedArrays(int vformat, boolean[] useInterleavedArrays, int[] iaFormat)
	{
		...
		switch (vformat)
		{
			...
			case (GeometryArray.COORDINATES | GeometryArray.NORMALS | GeometryArray.COLOR | GeometryArray.WITH_ALPHA | GeometryArray.TEXTURE_COORDINATE_2):
				iaFormat[0] = GL2.GL_T2F_C4F_N3F_V3F;
				break;
			default:
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.executeTexture()");
		...
		for (int i = 0; i < numActiveTexUnit; i++)
		{
			...
			if ((tus < texCoordSetMapLen) && (texCoordSetMapOffset[tus] != -1))
			{
				...
				enableTexCoordPointer(gl, i, texSize, GL.GL_FLOAT, bstride, verts);
			}
			else
			{
			...
			float[] varray, FloatBuffer varrayBuffer, float[] carray, int cDirty)
	{
				...
				|| geo_type == GeometryRetained.GEO_TYPE_LINE_STRIP_SET)
		{
			...
			else
			{
				...
				if ((vformat & GeometryArray.TEXTURE_COORDINATE) != 0)
				{
					executeTexture(texCoordSetMapLen, texSize, bstride, texCoordoff, texCoordSetMapOffset, numActiveTexUnitState, verts, gl);
				}
	...
	void executeIndexedGeometryVABuffer(Context ctx, GeometryArrayRetained geo, int geo_type, boolean isNonUniformScale,
			...
			int[] vertexAttrSizes, Object[] vertexAttrData, int texCoordMapLength, int[] texcoordoffset, int numActiveTexUnitState,
			...
			int imageFormat, Object imageBuffer, int depthFormat, Object depthBuffer)
	{
		...
		if ((type & Raster.RASTER_COLOR) != 0)
		{
			...
			if (imageDataType == ImageComponentRetained.IMAGE_DATA_TYPE_BYTE_ARRAY)
			{
				...
				gl.glReadPixels(xSrcOffset, yAdjusted, width, height, oglFormat, GL.GL_UNSIGNED_BYTE, ByteBuffer.wrap((byte[]) imageBuffer));
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexCoordGeneration()");
	...
	{ GL2.GL_SOURCE0_RGB, GL2.GL_SOURCE1_RGB, GL2.GL_SOURCE2_RGB, };

	private static final int[] _gl_combineAlphaSrcIndex =
	{ GL2.GL_SOURCE0_ALPHA, GL2.GL_SOURCE1_ALPHA, GL2.GL_SOURCE2_ALPHA, };

	private static final int[] _gl_combineRgbOpIndex =
	{ GL2.GL_OPERAND0_RGB, GL2.GL_OPERAND1_RGB, GL2.GL_OPERAND2_RGB, };

	private static final int[] _gl_combineAlphaOpIndex =
	{ GL2.GL_OPERAND0_ALPHA, GL2.GL_OPERAND1_ALPHA, GL2.GL_OPERAND2_ALPHA, };
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.bindTexture3D()");
			...
			int boundaryWidth, int dataType, Object data, boolean useAutoMipMap)
	{
		...
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DImage()");
				...
				|| (dataType == ImageComponentRetained.IMAGE_DATA_TYPE_BYTE_BUFFER))
		{
			...
			if (dataType == ImageComponentRetained.IMAGE_DATA_TYPE_BYTE_ARRAY)
			{
				...
				gl.glTexImage3D(GL2.GL_TEXTURE_3D, level, internalFormat, width, height, depth, boundaryWidth, format, GL.GL_UNSIGNED_BYTE,
				...
				|| (dataType == ImageComponentRetained.IMAGE_DATA_TYPE_INT_BUFFER))
		{
			...
			{
				gl.glTexImage3D(GL2.GL_TEXTURE_3D, level, internalFormat, width, height, depth, boundaryWidth, format, type,
						IntBuffer.wrap((int[]) data));
			}
			else
			{
				gl.glTexImage3D(GL2.GL_TEXTURE_3D, level, internalFormat, width, height, depth, boundaryWidth, format, type, (Buffer) data);
			}
			...
			Object data, boolean useAutoMipMap)
	{
		...
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DSubImage()");
				...
				|| (dataType == ImageComponentRetained.IMAGE_DATA_TYPE_BYTE_BUFFER))
		{
			...
			buf.position(offset);
			gl.glTexSubImage3D(GL2.GL_TEXTURE_3D, level, xoffset, yoffset, zoffset, width, height, depth, format, GL.GL_UNSIGNED_BYTE, buf);
				...
				|| (dataType == ImageComponentRetained.IMAGE_DATA_TYPE_INT_BUFFER))
		{
			...
			buf.position(offset);
			gl.glTexSubImage3D(GL2.GL_TEXTURE_3D, level, xoffset, yoffset, zoffset, width, height, depth, format, type, buf);
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DLodRange()");

		updateTextureLodRange(ctx, GL2.GL_TEXTURE_3D, baseLevel, maximumLevel, minimumLod, maximumLod);
	}

	void updateTexture3DLodOffset(Context ctx, float lodOffsetS, float lodOffsetT, float lodOffsetR)
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DLodOffset()");

		updateTextureLodOffset(ctx, GL2.GL_TEXTURE_3D, lodOffsetS, lodOffsetT, lodOffsetR);
	}
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DBoundary()");

		updateTextureBoundary(ctx, GL.GL_TEXTURE_2D, boundaryModeS, boundaryModeT, boundaryModeR, boundaryRed, boundaryGreen, boundaryBlue,
				boundaryAlpha);
	}
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DFilterModes()");

		updateTextureFilterModes(ctx, GL2.GL_TEXTURE_3D, minFilter, magFilter);
	}

	void updateTexture3DSharpenFunc(Context ctx, int numSharpenTextureFuncPts, float[] sharpenTextureFuncPts)
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DSharpenFunc()");

		updateTextureSharpenFunc(ctx, GL2.GL_TEXTURE_3D, numSharpenTextureFuncPts, sharpenTextureFuncPts);
	}

	void updateTexture3DFilter4Func(Context ctx, int numFilter4FuncPts, float[] filter4FuncPts)
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DFilter4Func()");

		updateTextureFilter4Func(ctx, GL2.GL_TEXTURE_3D, numFilter4FuncPts, filter4FuncPts);
	}

	void updateTexture3DAnisotropicFilter(Context ctx, float degree)
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.updateTexture3DAnisotropicFilter()");
			...
			int height, int boundaryWidth, int dataType, Object data, boolean useAutoMipMap)
	{
				...
				|| (dataType == ImageComponentRetained.IMAGE_DATA_TYPE_BYTE_BUFFER))
		{
			...
			else
			{
				...
				{
					gl.glTexImage2D(target, level, internalFormat, width, height, boundaryWidth, format, GL.GL_UNSIGNED_BYTE, (Buffer) data);
				}
				//////////////////////////////////////////////PJPJPJPJPJPJ//////////////////////////////////////
			}
		}
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.accum()");
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.accumReturn()");
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.clearAccum()");
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.decal1stChildSetup()");
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.decalNthChildSetup()");
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.decalReset()");
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.setGlobalAlpha()");
		// FIXME: SUN_global_alpha
		//        GL gl = context(ctx).getGL();
		//        if (gl.isExtensionAvailable("GL_SUN_global_alpha")) {
		//            gl.glEnable(GL.GL_GLOBAL_ALPHA_SUN);
			...
			float mapMinY, float mapMaxY, float mapZ, float alpha, boolean useBilinearFilter)
	{
		...
		// Temporarily disable fragment and most 3D operations
		gl.glPushAttrib(GL2.GL_ENABLE_BIT | GL2.GL_TEXTURE_BIT | GL2.GL_POLYGON_BIT | GL2.GL_CURRENT_BIT);
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.newDisplayList()");
		if (displayListId <= 0)
		{
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.endDisplayList()");
		GL2 gl = context(ctx).getGL().getGL2();
		gl.glEndList();
	}
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.callDisplayList()");
		if (id <= 0)
		{
	...
	{
		if (VERBOSE)
			System.err.println("JoglPipeline.freeDisplayList()");
		if (id <= 0)
		{
	...
	boolean initTexturemapping(Context ctx, int texWidth, int texHeight, int objectId)
	{
		...
		gl.glTexImage2D(GL2.GL_PROXY_TEXTURE_2D, 0, GL.GL_RGBA, texWidth, texHeight, 0, glType, GL.GL_UNSIGNED_BYTE, null);
	...
	GraphicsConfiguration getBestConfiguration(GraphicsConfigTemplate3D gct, GraphicsConfiguration[] gc)
	{
		...
		AbstractGraphicsScreen screen = (device != null) ? AWTGraphicsScreen
				.createScreenDevice(device, AbstractGraphicsDevice.DEFAULT_UNIT) : AWTGraphicsScreen.createDefault();
		...
		if (gct.getDoubleBuffer() == GraphicsConfigTemplate.REQUIRED && gct.getSceneAntialiasing() == GraphicsConfigTemplate.REQUIRED)
		{
		...
		GraphicsConfigurationFactory gcFactory = GraphicsConfigurationFactory.getFactory(AWTGraphicsDevice.class,
		...
		if (awtConfig != null && chooser.isCalled() == false)
		{
			...
			boolean isDoubleBuffering = (gct.getDoubleBuffer() == GraphicsConfigTemplate.REQUIRED || gct.getDoubleBuffer() == GraphicsConfigTemplate.PREFERRED);

			caps.setDoubleBuffered(isDoubleBuffering);

			if (isDoubleBuffering
					&& (gct.getSceneAntialiasing() == GraphicsConfigTemplate.REQUIRED || gct.getSceneAntialiasing() == GraphicsConfigTemplate.PREFERRED))
			{
				caps.setSampleBuffers(true);
	...
	private static AWTGraphicsConfiguration getAWTGraphicsConfiguration(GraphicsConfigurationFactory gcFactory,
			CapabilitiesImmutable capsRequested, CapabilitiesChooser chooser, AbstractGraphicsScreen screen)
	{
		return (AWTGraphicsConfiguration) gcFactory.chooseGraphicsConfiguration(capsRequested, capsRequested, chooser, screen,
	...
	private static final class J3DCapsChooser implements GLCapabilitiesChooser
	{
		...
		public int chooseCapabilities(final CapabilitiesImmutable desiredCaps,
				final List<? extends CapabilitiesImmutable> availableCapsList, final int windowSystemRecommendedChoice)
		{
			called = true;
