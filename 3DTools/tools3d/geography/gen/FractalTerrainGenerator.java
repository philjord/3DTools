...
/**
 * class over ridden to allow acces to private member
 * @author pj
 *
...
public class FractalTerrainGenerator extends GeometryGenerator
{
	...
	{
		this(DEFAULT_SIZE, DEFAULT_SIZE, DEFAULT_HEIGHT, true, DEFAULT_SEALEVEL, DEFAULT_ITERATIONS, DEFAULT_ROUGHNESS, 0, null);
	}
	...
	{
		this(DEFAULT_SIZE, DEFAULT_SIZE, DEFAULT_HEIGHT, true, DEFAULT_SEALEVEL, DEFAULT_ITERATIONS, DEFAULT_ROUGHNESS, 0, terrain);
	}
	...
	{
		this(DEFAULT_SIZE, DEFAULT_SIZE, DEFAULT_HEIGHT, useSea, seaLevel, DEFAULT_ITERATIONS, DEFAULT_ROUGHNESS, 0, null);
	}
	 ...
	 *    not square
	 */
	public FractalTerrainGenerator(float[][] terrain, boolean useSea, float seaLevel)
	{
		this(DEFAULT_SIZE, DEFAULT_SIZE, DEFAULT_HEIGHT, useSea, seaLevel, DEFAULT_ITERATIONS, DEFAULT_ROUGHNESS, 0, terrain);
	}
	...
	{
		this(width, depth, DEFAULT_HEIGHT, true, DEFAULT_SEALEVEL, DEFAULT_ITERATIONS, DEFAULT_ROUGHNESS, 0, null);
	}
	...
	{
		this(width, depth, DEFAULT_HEIGHT, true, DEFAULT_SEALEVEL, DEFAULT_ITERATIONS, DEFAULT_ROUGHNESS, 0, terrain);
	}
	 ...
	 */
	public FractalTerrainGenerator(float height, int iterations, float roughness, long seed)
	{
		this(DEFAULT_SIZE, DEFAULT_SIZE, height, true, DEFAULT_SEALEVEL, iterations, roughness, seed, null);
	}
	 ...
	 *     constructors
	 */
	public FractalTerrainGenerator(float width, float depth, float height, int iterations, float roughness, long seed)
	{
		this(width, depth, height, true, DEFAULT_SEALEVEL, iterations, roughness, seed, null);
	}
	 ...
	 *     constructors
	 */
	public FractalTerrainGenerator(float width, float depth, float height, boolean useSea, float seaLevel, int iterations, float roughness,
			long seed, float[][] terrain)
	{
		if ((width <= 0) || (height <= 0))
		...
		gridGenerator = new ElevationGridGenerator(terrainWidth, terrainDepth, side_points, side_points);
	 ...
	 * @param iterations The number of subdivisions to calculate
	 * @param roughness Division factor for each iteration of height
	 * @param seed is a random seed I would guess
	 */
	public void setGenerationFactors(float height, int iterations, float roughness, long seed)
	{
		...
		gridGenerator.setDimensions(terrainWidth, terrainDepth, side_points, side_points);
	...
	private float[][] subdivideSurface()
	{
		...
		{
			old = 0;
			new_points = (old_points * 2) - 1; // (int)Math.pow(2, (k - 1)); // see also calcSidePoints
			...
			for (i = 0; i < new_points; i++)
			{
				...
				else
				{
					...
					for (j = 0; j < old_points - 1; j++)
					{
						...
						sign = randomiser.nextBoolean() ? 1 : -1;
						height = (old_surface[old - 1][j] + old_surface[old - 1][j + 1] + old_surface[old][j] + old_surface[old][j + 1]) / 4;
						height += delta * randomiser.nextFloat() * sign;
	...
	private final int calcSidePoints(int itrs)
	{
		...
		for (int i = 0; i < itrs; i++)
			points = (points * 2) - 1;
		// points += (int)Math.pow(2, i); // fixed similarly to  new_points = (old_points * 2) - 1;// (int)Math.pow(2, (k - 1)); above
